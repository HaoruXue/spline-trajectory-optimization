...
def dynamics(model_dict, x, u):
    ...
    Fy_fl = mu * Fz_fl * (1 + eps_f * Fz_fl / Fz0_f) * ca.sin(Cf *
                                                              ca.arctan(Bf * a_fl - Ef * (Bf * a_fl - ca.arctan(Bf * a_fl))))
    Fy_fr = mu * Fz_fr * (1 + eps_f * Fz_fr / Fz0_f) * ca.sin(Cf *
                                                              ca.arctan(Bf * a_fr - Ef * (Bf * a_fr - ca.arctan(Bf * a_fr))))
    Fy_rl = mu * Fz_rl * (1 + eps_r * Fz_rl / Fz0_r) * ca.sin(Cr *
                                                              ca.arctan(Br * a_rl - Er * (Br * a_rl - ca.arctan(Br * a_rl))))
    Fy_rr = mu * Fz_rr * (1 + eps_r * Fz_rr / Fz0_r) * ca.sin(Cr *
                                                              ca.arctan(Br * a_rr - Er * (Br * a_rr - ca.arctan(Br * a_rr))))
    ...
    # cg position
    x_dot = v_dot * ca.cos(phi)
    y_dot = v_dot * ca.sin(phi)
...
def add_constraints(model_dict, opti, x, u, t, uip1):
    # tyre constraints
    phi = x[2]  # yaw
    omega = x[3]  # yaw rate
    beta = x[4]  # slip angle
    ...
    opti.subject_to(ca.power(fd * fb, 2) <= 1e-6)
